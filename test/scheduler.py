...
# from .load_data import populate_db

# async def fetch_and_save():
#     await populate_db()

# scheduler = AsyncIOScheduler()
# scheduler.add_job(fetch_and_save, "interval", minutes=5)
import reflex as rx
import sqlite3
import pandas as pd
...
class State(rx.State):
    ...
    def get_all_tickers(self) -> List[Dict]:
        ...
        if self.selected_exchange != 'All': query.append(f"AND exchange = '{self.selected_exchange}'")
        
        # Order by condition
        if self.selected_cond == "Market Cap": order_by_clause = "ORDER BY market_cap DESC"
        if self.selected_cond == '% Increase': order_by_clause = "ORDER BY pct_price_change DESC"
    ...
    @rx.var 
    def current_filters(self) -> str:
        filters: List[str] = []
        if self.selected_category != 'All': filters.append(self.selected_category)
        if self.selected_industry != 'All': filters.append(self.selected_industry)
        if self.selected_cond != 'All': filters.append(self.selected_cond)
        if self.selected_exchange != 'All': filters.append(self.selected_exchange)
    ...
    def clear_all_filter(self):
        self.selected_category = self.selected_cond = self.selected_industry = self.selected_exchange = "All"
...
def ticker_filter():
    return rx.box(
        rx.popover.root(
            rx.popover.trigger(
                rx.button(rx.icon("filter", size=20), variant='outline'),
            ),
            rx.popover.content(
                rx.vstack(
                    fundamentals_filter(),
    ...
    return rx.box(
        rx.hstack(
            ...
            rx.menu.root(
                rx.menu.trigger(
                    rx.button(
                        rx.cond(State.current_filters != "",
                                State.current_filters,
                                rx.icon("filter", size=15),
                            ),
                        variant=rx.cond(State.current_filters != "", 'solid', 'outline')
                    )
                ),
                rx.menu.content(
                    rx.menu.item(
                        rx.text("Clear", weight="medium"),
                        on_select=State.clear_all_filter,
                    ),
                    
                    # Exchange
                    rx.menu.sub(
                        rx.menu.sub_trigger("Exchange"),
                        rx.menu.sub_content(
                            rx.foreach(
                                State.platforms,
                                lambda platform: rx.menu.item(
                                    rx.hstack(
                                        ...
                                        rx.text(platform, weight=rx.cond(platform == State.selected_exchange, "bold", "normal")),
                                        spacing="2",
                                        align="center",
                                    ),
                    ...
                    # Conditions
                    rx.menu.sub(
                        rx.menu.sub_trigger("Conditions"),
                        rx.menu.sub_content(
                            rx.foreach(
                                State.filters,
                                lambda filter: rx.menu.item(
                                    rx.hstack(
                                        rx.cond(
                                            filter == State.selected_cond,
                                            rx.icon("check", size=16),
                                            rx.box(width="16px")
                                        ),
                                        rx.text(filter, weight=rx.cond(filter == State.selected_cond, "bold", "normal")),
                                        spacing="2",
                                        align="center",
                                    ),
