...
def load_price_board(tickers: list[str]) -> pd.DataFrame:
    price_board: pd.DataFrame = Trading(source="vci").price_board(symbols_list=tickers)
    price_board.columns = price_board.columns.droplevel(0)

    # Compute instrument
    if "match_price" in price_board.columns:
        price_board = price_board.rename(columns={"match_price": "current_price"})
        price_board["price_change"] = (
            price_board["current_price"] - price_board["ref_price"]
        )
        price_board["pct_price_change"] = (
            price_board["price_change"] / price_board["ref_price"]
        ) * 100

    else:
        price_board = price_board.rename(columns={"ref_price": "current_price"})
        price_board["price_change"] = 0
        price_board["pct_price_change"] = 0

    # Normalize
    price_board["current_price"] = round(price_board["current_price"] * 1e-3, 2)
...
def main() -> None:
    ...
    price_board: pd.DataFrame = load_price_board(tickers=comparison.ticker.tolist())

    result: pd.DataFrame = pd.merge(
        left=comparison, right=price_board, left_on="ticker", right_on="symbol"
    )
