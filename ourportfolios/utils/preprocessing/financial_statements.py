...
# 🔧 FIXED FUNCTION: HANDLE MULTIINDEX RATIOS PROPERLY
...
def comprehensive_financial_transformation_fixed(ticker_symbol, source='VCI', period='year', lang='en'):
    """
    FIXED VERSION: Properly handles MultiIndex ratios columns
    """
    
    # Helper function for YoY growth calculation
    ...
    bank_indicators = ['Net Interest Income', 'Interest and Similar Income', 'Net Fee and Commission Income', 'Provision for credit losses']
    bank_column_count = sum(1 for col in bank_indicators if col in income_statement.columns)
    is_bank = bank_column_count >= len(bank_indicators) // 2
    
    print(f"Company type: {'Bank' if is_bank else 'Non-Bank'}")
...
# Apply income statement mappings
    ...
    if is_bank:
        ...
        transformed_income['EPS'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'EPS (VND)')
        transformed_income['Outstanding Share'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'Outstanding Share (Mil. Shares)')
    else:
        # Add ratio-based columns for non-banks using MultiIndex access
        transformed_income['EPS'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'EPS (VND)')
        transformed_income['Outstanding Share'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'Outstanding Share (Mil. Shares)')
        transformed_income['EBITDA'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'EBITDA (Bn. VND)')
        transformed_income['EBIT'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'EBIT (Bn. VND)')
    ...
    if is_bank:
        ...
        balance_mapping = {
            ...
            'Trading securities': 'Trading Securities, net',
            ...
            'Loans to customers': 'Loans and advances to customers, net',
    ...
    operating_cf = transformed_cash_flow.get('Operating cash flow', pd.Series(dtype=float))
    ...
    if not operating_cf.isna().all() and not capex.isna().all():
        transformed_cash_flow['Free cash flow'] = operating_cf.fillna(0) + capex.fillna(0)
    else:
    ...
    if is_bank:
        ...
        leverage_liquidity['Year'] = safe_get_multiindex(key_ratios, 'Meta', 'yearReport')
        efficiency['Year'] = safe_get_multiindex(key_ratios, 'Meta', 'yearReport')
        
        if period == 'quarter':
            per_share['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')
            growth_rate['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')
            profitability['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')
            valuation['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')
            leverage_liquidity['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')
            efficiency['Quarter'] = safe_get_multiindex(key_ratios, 'Meta', 'lengthReport')

        # Per Share Value
        per_share['Earnings'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'EPS (VND)')
        
        outstanding_shares = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'Outstanding Share (Mil. Shares)')
        ...
        if not operating_cf.isna().all() and not outstanding_shares.isna().all():
            ...
            per_share['Free Cash Flow'] = fcf_total / outstanding_shares.replace(0, pd.NA)
        else:
            per_share['Free Cash Flow'] = pd.NA
        
        # Dividend per share
        if not dividends_paid.isna().all() and not outstanding_shares.isna().all():
            per_share['Dividend'] = (-dividends_paid) / outstanding_shares.replace(0, pd.NA)
        else:
            per_share['Dividend'] = pd.NA
        
        per_share['Book Value'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'BVPS (VND)')
        
        # Growth Rates
        growth_rate['Earnings YoY'] = calculate_yoy_growth(per_share['Earnings'])
        growth_rate['Free Cash Flow YoY'] = calculate_yoy_growth(per_share['Free Cash Flow'])
        ...
        # Profitability
        profitability['Net Margin'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'Net Profit Margin (%)')
        profitability['ROE'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'ROE (%)')
        ...
        valuation['P/Cash Flow'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu định giá', 'P/Cash Flow')
        ...
        leverage_liquidity['Financial Leverage'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Financial Leverage')
        
        # Efficiency
        efficiency['ROA'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'ROA (%)')
        
        attributable_profit = safe_get(transformed_income, 'Attributable to parent company')
        if not dividends_paid.isna().all() and not attributable_profit.isna().all():
            efficiency['Dividend Payout %'] = (-dividends_paid / attributable_profit.replace(0, pd.NA)) * 100
        else:
        ...
        # Profitability
        profitability['Gross Margin'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'Gross Profit Margin (%)')
        
        operating_profit = safe_get(transformed_income, 'Operating income')
        if not operating_profit.isna().all() and not net_sales.isna().all():
            profitability['Operating Margin'] = (operating_profit / net_sales.replace(0, pd.NA)) * 100
        else:
            profitability['Operating Margin'] = pd.NA
        
        profitability['Net Margin'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'Net Profit Margin (%)')
        profitability['ROE'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'ROE (%)')
        profitability['ROIC'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'ROIC (%)')
        
        # ROCE calculation
        ebit = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'EBIT (Bn. VND)')
        ...
        if not ebit.isna().all() and not total_assets.isna().all() and not current_liabilities.isna().all():
        ...
        # Margin calculations
        ebitda = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'EBITDA (Bn. VND)')
        if not ebitda.isna().all() and not net_sales.isna().all():
            profitability['EBITDA Margin'] = (ebitda / net_sales.replace(0, pd.NA)) * 100
        else:
            profitability['EBITDA Margin'] = pd.NA
        
        profitability['EBIT Margin'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'EBIT Margin (%)')
        ...
        # Leverage & Liquidity
        leverage_liquidity['Debt/Equity'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu cơ cấu nguồn vốn', 'Debt/Equity')
        ...
        leverage_liquidity['Short and Long Term Borrowings to Equity'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu cơ cấu nguồn vốn', '(ST+LT borrowings)/Equity')
        leverage_liquidity['Financial Leverage'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Financial Leverage')
        leverage_liquidity['Quick Ratio'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Quick Ratio')
        leverage_liquidity['Current Ratio'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Current Ratio')
        leverage_liquidity['Cash Ratio'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Cash Ratio')
        leverage_liquidity['Interest Coverage'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu thanh khoản', 'Interest Coverage')
        
        # Efficiency
        efficiency['Asset Turnover'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu hiệu quả hoạt động', 'Asset Turnover')
        efficiency['Inventory Turnover'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu hiệu quả hoạt động', 'Inventory Turnover')
        efficiency['ROA'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu khả năng sinh lợi', 'ROA (%)')
        ...
        efficiency['Cash Conversion Cycle'] = safe_get_multiindex(key_ratios, 'Chỉ tiêu hiệu quả hoạt động', 'Cash Cycle')
...
# 🎯 UPDATED FUNCTION: SHOW PROCESSED TABLES WITH FIXED RATIOS
def show_processed_tables_fixed(ticker, period='year'):
    """
    🎯 Display all TRANSFORMED/PROCESSED financial tables with FIXED ratios
    Now properly handles MultiIndex ratios columns
    
    Usage: 
show_processed_tables_fixed('FPT')           # Yearly data
        show_processed_tables_fixed('FPT', 'quarter') # Quarterly data
    """
    print(f"🔥 PROCESSED FINANCIAL DATA FOR {ticker.upper()} ({period.upper()} DATA) (FIXED VERSION)")
    print("=" * 80)
    
    # Use the FIXED comprehensive transformation system
    print(f"🔄 Processing {ticker.upper()} with FIXED transformation ({period} data)...")
    data = comprehensive_financial_transformation_fixed(ticker, period=period)
    
    if data:
        company_type = 'Bank' if data['is_bank'] else 'Non-Bank'
        print(f"🏛️ Company Type: {company_type}")
        
        # 1. Transformed Income Statement
        print(f"\n📈 1. TRANSFORMED INCOME STATEMENT")
        print("-" * 60)
        display(data['transformed_income_statement'])
        
        # 2. Transformed Balance Sheet
        print(f"\n💰 2. TRANSFORMED BALANCE SHEET")
        print("-" * 60)
        display(data['transformed_balance_sheet'])
        
        # 3. Transformed Cash Flow Statement
        print(f"\n💸 3. TRANSFORMED CASH FLOW STATEMENT")
        print("-" * 60)
        display(data['transformed_cash_flow'])
        
        # 4. Categorized Key Ratios (show all categories)
        print(f"\n🔢 4. CATEGORIZED KEY RATIOS (FIXED)")
        print("-" * 60)
        
        for category_name, ratios_df in data['categorized_ratios'].items():
            print(f"\n📊 {category_name.upper()}:")
            print("-" * 40)
            if not ratios_df.empty:
                # Show only non-NA columns
                non_na_cols = ratios_df.columns[ratios_df.notna().any()].tolist()
                if non_na_cols:
                    display(ratios_df[non_na_cols])
                else:
                    print("All values are NA")
            else:
                print("No data available for this category")
        
        print(f"\n✅ COMPLETE! All transformed tables displayed for {ticker.upper()}")
        return data
...
# 🎯 SIMPLE VERSION
def analyze_fixed(ticker, period='year'):
    """
    🎯 Super simple - just type: analyze_fixed('FPT')
    Shows all transformed/processed tables with FIXED ratios

    Usage:
        analyze_fixed('FPT')           # Yearly data
        analyze_fixed('FPT', 'quarter') # Quarterly data
    """
    return show_processed_tables_fixed(ticker, period=period)
